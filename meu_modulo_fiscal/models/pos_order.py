...
class PosSession(models.Model):
    ...
    def _load_pos_data(self, data):
        """
        Sobrescreve o carregamento da sessão para injetar o 'seed' de numeração
        da contingência offline. Garante que mesmo em PC novo ou com localStorage
        limpo, o JS nunca emitirá um número abaixo do histórico da SEFAZ.

        Arquitetura: Série por Caixa (DEC-001) isola cada PDV.
        Este seed protege o PDV contra colisão CONSIGO MESMO ao trocar de máquina.
        """
        result = super()._load_pos_data(data)

        # Série de contingência do caixa: 600 + ID do pos.config
        config_id = self.config_id.id
        serie_contingencia = str(600 + config_id)

        # Busca o maior número fiscal já emitido nessa série (online + contingência)
        # Usa ordem numérica explícita para evitar ordenação lexicográfica ('9' > '10')
        ultimo_pedido = self.env['pos.order'].sudo().search(
            [('x_fiscal_serie', '=', serie_contingencia)],
            order='id desc',
            limit=50  # Pega os 50 mais recentes e filtra numericamente no Python
        )

        ultimo_numero = 0
        for pedido in ultimo_pedido:
            try:
        ...
        _logger.info(
            '[CONTINGENCIA-SEED] Caixa ID=%d | Série=%s | Último número emitido: %d',
